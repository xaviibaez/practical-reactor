...
public class c2_TransformingSequence extends TransformingSequenceBase {
    ...
    public void transforming_sequence_2() {
        ...
        Flux<String> resultSequence = numbersFlux.map(this::transform);
        numbersFlux.doOnNext(System.out::println).subscribe();
    ...
    private String transform(Integer x) {
        if (x > 0) return ">";
        if (x == 0) return "=";
        return "<";
    }
     ...
     * to result sequence.
     * <p>
     * Do not alter `numerical_service` implementation! XD
